...
function handleToDoSubmit(event){
    ...
    const newTodo = toDoInput.value;    //input의 현재 value를 새로운 변수에 복사   (1. input에서 value를 얻어서)
    toDoInput.value=""; //newToDo 변수와 상관없음.
    const newTodoObj ={
        text:newTodo,
        id: Date.now(), //id로 각각의 li item을 구별할 수 있음.
    }
    toDos.push(newTodoObj); //paintToDo를 하기전에 toDos 라는 배열에 newTodoObj push (newTodoObj는 text만 저장하는게 아니라 id도 같이 저장)
    paintToDo(newTodoObj); //(2. paintToDo 라는 새로운 function에 값을 보냄)
    saveToDos();
}
